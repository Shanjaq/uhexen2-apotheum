void() gravwell_think = {
	if (self.cnt == 95) {
		stuffcmd(self.catapulter, "fov 80\n");
	}
	if (self.cnt == 100) {
		stuffcmd(self.catapulter, "fov 70\n");
	}
	if (self.cnt == 105) {
		stuffcmd(self.catapulter, "fov 60\n");
	}
	if (self.cnt == 110) {
		stuffcmd(self.catapulter, "fov 50\n");
	}
	if (self.cnt == 115) {
		stuffcmd(self.catapulter, "fov 40\n");
	}
	if (self.cnt == 120) {
		stuffcmd(self.catapulter, "fov 30\n");
	}
	if (self.cnt == 125) {
		remove(self);
	}
...
void() baddie_fade = {
	...
	if (self.halted == 1) {
		self.halted = 2;
		self.dest = vectoangles (self.catapulter.origin - self.origin);
		self.dest_x -= 90;
	}
	...
	if (self.classname != "player") {
		...
		newmis = spawn();
		newmis.catapulter = self.catapulter;
...
//		if((grap>0) && (head != world) && (head.solid != SOLID_BSP) && (head.classname != "info_player_start") && (head.classname != "info_player_coop") && (head.classname != "info_player_deathmatch") && (head.classname != "info_intermission"))
		...
		if((head != world) && (head.solid != SOLID_BSP) && (head.takedamage == DAMAGE_YES) && (head.health > -1.00000))
		{
			...
			//if ((head.classname != "player") && (vlen(head.origin - self.origin) < 360) && (head.halted == 0)) {
			if ((vlen(head.origin - self.origin) < 360) && (head.halted == 0)) {
...
void() bh_ball_think = {
	if (self.origin_z < (self.hit_z + 40)) {
		self.velocity_z = 0;
		self.cnt = 0;
