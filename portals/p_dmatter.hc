...
void() baddie_bomb = {
	...
	newmis.abslight = 2;
	newmis.avelocity = randomv('-200 -200 -200', '200 200 200');
	newmis.owner = self;
...
void() dark_matter_effects_think = {
	if (time < self.splash_time)
	{
		...
		self.velocity += randomv('-80 -80 0', '80 80 0');
		self.scale = ((1.00000 - ((self.splash_time - time) / self.lifetime)) * self.lip);
...
void() dark_matter_effects = {
	...
	newmis.solid = SOLID_NOT;
	newmis.movetype = MOVETYPE_NOCLIP;
	setorigin(newmis, (self.origin + (randomv('-200 -200 -20', '200 200 10') * newmis.spellradiusmod)));
	newmis.drawflags (+) (DRF_TRANSLUCENT | MLS_ABSLIGHT);
	newmis.abslight = 0.06;
	...
	newmis.avelocity = randomv('-400.00000 -400.00000 -400.00000','400.00000 400.00000 400.00000');
	newmis.lip = (random(0.53000, 1.46000)*self.spellradiusmod);
...
void() energy_field = {
	...
	newmis.solid = SOLID_NOT;
	newmis.scale = 0.1;
	newmis.avelocity = randomv('-400 -400 -400', '400 400 400');
...
void() dark_matter_spread = {
	if (time < self.splash_time) {
		...
		particle2 ( (self.origin + randomv('-300 -300 -30', '300 300 30')), '-30.00000 -30.00000 50.00000', '30.00000 30.00000 100.00000', random(1, 6), 2, 80.00000);
		thinktime self : 0.1;
...
void() dark_matter_fall = {
	if (self.origin_z > self.hit_z) {
		//if ((self.origin_z - 40) < self.hit_z) {
		//self.velocity_z *= 0.8;
		//}
		if (self.angles_z != 180) {
			...
			self.velocity_z = self.hit_z - self.origin_z;
		}
		
		self.scale = ((1.6 * self.spellradiusmod) * (1.00000 - ((self.origin_z - self.hit_z) / (self.height - self.hit_z))));
...
void() light_matter_rise = {
	...
	//newmis.lip = (self.origin_z + (200 - (self.spellradiusmod * 12)));
	newmis.hit_z = (self.origin_z + (128 * self.spellradiusmod));
	newmis.avelocity_y = -100;
	newmis.velocity_z = (newmis.hit_z - newmis.origin_z);
	thinktime newmis : 0.02;
...
void() dark_matter_init = {
	...
	newmis.solid = SOLID_NOT;
	newmis.movetype = MOVETYPE_NOCLIP;
	//newmis.hit_z = trace_endpos_z + (190 - (self.spellradiusmod * 12));
	newmis.hit_z = (trace_endpos_z + (128 * self.spellradiusmod));
	traceline (self.origin , (self.origin+('0 0 600')) , TRUE , self);
	setorigin(newmis, trace_endpos);
	newmis.velocity = '0 0 0';
	newmis.height = newmis.origin_z;
	newmis.drawflags = MLS_ABSLIGHT;
	thinktime newmis : 1;
