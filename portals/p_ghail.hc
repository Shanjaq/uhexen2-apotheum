...
void ()make_solidity = {
	if (self.origin_z > self.hit_z) {
		...
		if (time < self.splash_time) {
			particle2 ( (randomv(self.absmin, self.absmax) - ('0 0 1' * self.size_z)), '-90.00000 -90.00000 150.00000', '90.00000 90.00000 300.00000', 255.00000, PARTICLETYPE_ICE, (random(24, 48) * self.scale));
			thinktime self : 0.01;
...
void() Glacier_fall = {
	...
	hail.hit_z = trace_endpos_z;

	hail.avelocity = randomv('-700.00000 -700.00000 -700.00000','700.00000 700.00000 700.00000');
...
void() glacier_spawner_think = {
	if (self.cnt>0) {
		self.cnt -= 1;
		Glacier_fall();
		thinktime self : random(0.01, 1.5);
