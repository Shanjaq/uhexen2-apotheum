void() toxic_cloud_part_think =
{
	...
	if (time < self.splash_time)
	{
		...
		pos = self.origin + randomv('-64 -64 0', '64 64 0');
		//particle2 ( (self.origin + randomv('-64 -64 -24', '64 64 8')), '-30.00000 -30.00000 -96.00000', '30.00000 30.00000 -128.00000', random(185, 191), 0, 1.00000);
		...
		thinktime self : random(0.16, 0.23);
	}
	else
	{
...
void(vector org) toxic_cloud_part = 
{
	...
	newmis.velocity = randomv('-12 -12 -3', '12 12 3');
...
void() toxic_cloud_think =
{
	...
	if (time < self.splash_time)
	{
		pos = (self.origin + randomv('-128 -128 -36', '128 128 36') * self.spellradiusmod);
		...
		if (i < 4)
		{
			...
			thinktime self : (random(0.22, 0.41) / self.spellradiusmod);
		}
	}
	else
	{
