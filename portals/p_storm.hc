...
void() storm_ball_think =
{
	...
	if (time < self.splash_time)
	{
		...
		{
			i = 0;
			self.dest = self.dest2 + randomv('-768 -768 -32', '768 768 -32');
			self.dest_z = self.origin_z + 32;
			while ((pointcontents(self.dest) != CONTENT_EMPTY) && (i < 4))
			{
				self.dest = self.dest2 + randomv('-768 -768 0', '768 768 0');
				self.dest_z = self.origin_z + 32;
				i += 1;
			}
...
void() storm_ball =
{
	...
	stormball.dest = (self.origin + randomv('-64 -64 -256', '64 64 -64'));
	...
	stormball.avelocity = randomv('-700.00000 -700.00000 -700.00000','700.00000 700.00000 700.00000');
	...
	thinktime stormball : HX_FRAME_TIME;
	...
	newmis.skin = 1;
	newmis.avelocity = randomv('-700.00000 -700.00000 -700.00000','700.00000 700.00000 700.00000');
...
void() storm_melee = {
	...
	{
		do_lightning ( self.owner, 3, STREAM_ATTACHED, 1.00000, self.origin + randomv('-100 -100 0', '100 100 20'), nearest.origin, self.spelldamage + random(self.spelldamage*(-0.250000), self.spelldamage*1.750000), TE_STREAM_LIGHTNING);
	}
	else
	{
		if (random() < 0.5)
		{
			self.dest2 = self.origin + (randomv('-256 -256 -10', '256 256 5') * self.spellradiusmod);
		...
		{
			self.dest2 = self.origin + (randomv('-128 -128 -10', '128 128 0') * self.spellradiusmod);
