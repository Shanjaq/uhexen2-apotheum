...
void  ()photon_splash_think =  {
	if ( (self.scale == 0.05000) ) {
		...
		pitch_roll_for_slope (VEC_ORIGIN, self);
...
void  (vector a)photon_splash =  {
	...
	newmis.frame = 1;
	//   setorigin ( newmis, (a + randomv('-40.00000 -40.00000 30.00000','40.00000 40.00000 30.00000')));
	setorigin ( newmis, a);
...
void  ()photon_beam_think =  {
	if ( (self.cnt < 95.00000) ) {
		...
		if ( (self.catapulter.origin_z > self.dest_z) ) {
			setorigin ( self.catapulter, (self.catapulter.origin + (normalize ( (self.dest - self.origin)) * self.wisdom)));
		} else {
			setorigin ( self.catapulter, self.origin);
		}
		
		T_RadiusDamageFlat (self.catapulter, self.owner, (self.spelldamage + random(self.spelldamage*(-0.12500), self.spelldamage*0.12500))*0.50000, 164.00000 * self.spellradiusmod, self.owner, FALSE);
		...
		if (self.cnt == 95) {
			if (self.catapulter != world)
			{
				thinktime self.catapulter : 0.02000;
				self.catapulter.think = SUB_Remove;
...
void  ()photon_ball_think =  {
	...
	if ( (vlen ( (self.dest - self.origin)) < 300.00000) ) {
		...
		if ( (vlen ( (self.dest - self.origin)) < PARTICLETYPE_SPELL) ) {
			...
			thinktime self : 0.02000;
			self.think = photon_beam;
		} else {

			thinktime self : 0.06000;
	...
	particle2 ( (self.origin + randomv('-30.00000 -30.00000 -30.00000','30.00000 30.00000 30.00000')), '-200.00000 -200.00000 -20.00000', '200.00000 200.00000 200.00000', 255.00000, 0.00000, 80.00000);
};
...
void  ()photon_ball =  {
	...
	newmis.dest = randomv(newmis.dest,trace_endpos);
	traceline ( newmis.dest, (newmis.dest - '0.00000 200.00000 0.00000'), TRUE, newmis);
	newmis.dest = randomv(newmis.dest,trace_endpos);
	...
	sound ( newmis, CHAN_AUTO, "beamhit4.wav", TRUE, ATTN_NORM);
	newmis.velocity = randomv('-300.00000 -300.00000 -500.00000','300.00000 300.00000 0.00000');
