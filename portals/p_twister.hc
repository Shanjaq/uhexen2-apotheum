...
void() junker_think = {
	...
	while (head) {
		...
		if ( ((head.takedamage == DAMAGE_YES) || (head.classname == "razorwind") || (head.classname == "aero")) && ((head.solid != SOLID_BSP) && (head.touch != puzzle_touch) && (head.touch != weapon_touch)) ) {
			if (((head.classname == "razorwind") || (head.classname == "aero"))&& (head.lip != 1)) {
				head.lip = 1;
			}
	...
	if (self.lifetime > 0) 
	{
		...
		particle2 ( (self.origin + randomv('-30 -30 0', '30 30 100')), '-30.00000 -30.00000 50.00000', '30.00000 30.00000 150.00000', random(15, 25), 7, 50.00000);
		particle2 ( (self.origin + randomv('-30 -30 0', '30 30 100')), '-30.00000 -30.00000 25.00000', '30.00000 30.00000 50.00000', random(15, 25), 2, 50.00000);
...
void() twister_missile = 
{
	...
	org = (self.origin + (randomv('-150 -150 -32', '150 150 -10') * self.spellradiusmod));
	...
	if (i < 4)
	{
		...
		newmis.velocity = randomv('-128.00000 -128.00000 -190.00000', '128.00000 128.00000 -490.00000');
...
void() twister_melee = {
	...
	self.dest2_z = self.lip;
	self.dest2 = (self.dest2 + (randomv('-500 -500 0', '500 500 196')* self.spellradiusmod));
	...
	while(head) {
		if (head.takedamage == DAMAGE_YES)
		{
			...
			head.velocity += randomv('-300 -300 0', '300 300 300');
		}
		else if (head.movetype == MOVETYPE_FLYMISSILE)
		{
			head.velocity += randomv('-300 -300 0', '300 300 0');
		}
		else if ((head.movetype == MOVETYPE_BOUNCEMISSILE) || (head.movetype == MOVETYPE_BOUNCE))
		{
			head.velocity += randomv('-300 -300 96', '300 300 128');
		}
		head = head.chain;
	}
};
