...
void ()lava_think = {
	if ((self.velocity_z > 0.00000) || (self.origin_z > self.hit_z))
	{
		self.velocity_z -= ((time - self.ltime) * 600);
		particle2 ( self.origin + randomv('-100 -100 -100', '100 100 100'), '-100.00000 -100.00000 -100.00000', '100.00000 100.00000 100.00000', 140.00000, 16, random(1, 15));
...
void ()volcano_erupt = {
	...
	while (self.cnt <= 4) {
		...
		newmis.hit_z = self.origin_z + 24.00000;
...
void ()volcano_start = {
	...
	sound (newmis, CHAN_AUTO, "dsamaln6.wav", 1, ATTN_NORM);
	thinktime newmis : random(1.0, 2.5);
	newmis.think = volcano_erupt;
	MonsterQuake2(600.0, newmis);
};