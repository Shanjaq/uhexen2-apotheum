...
void() supernova_starfield_think =
{
	...
	if (time < self.splash_time)
	{
		...
		//particle2 ( trace_endpos, (v_forward * randomv((len * -1.00000), -64.00000)) + '-12 -12 -12', (v_forward * random((len * -1.00000), -64.00000)) + '12 12 12', random(25, 31), PARTICLETYPE_BLOB, random(15.00000, 37.00000));
		//particle2 ( trace_endpos, (v_forward * randomv((len * -1.00000), -64.00000)) + '-12 -12 -12', (v_forward * random((len * -1.00000), -64.00000)) + '12 12 12', random(25, 31), PARTICLETYPE_BLOB, random(15.00000, 37.00000));
		...
		thinktime self : random((HX_FRAME_TIME * 2.00000), (HX_FRAME_TIME * 6.00000));
	}
	else
	{
		if (self.goalentity != world)
...
void () supernova_star_think =
{
	...
	thinktime self : HX_FRAME_TIME * 2.00000;
};
...
void() supernova_think =
{
	...
	if (time < self.splash_time)
	{
		if (self.origin_z > self.lip)
		{
			self.velocity = '0 0 0';
			self.dest = self.origin;
			self.dest_z = self.lip;
		...
		if (self.camptime == 0)
		{
			if (self.attack_finished < time)
			{
				...
				while(found != world)
				{
					...
					self.dest = randomv(found.absmin, found.absmax);
					...
					if (found.halted == 0)
					{
						if ((found.flags & FL_MONSTER) && ((!(found.flags2 & FL_ALIVE)) || found.health <= self.spelldamage * 1.12500))
						{
							...
							newmis.angles_x = newmis.angles_x = ((90.00000 - newmis.angles_x) * -1.00000);
							newmis.abslight = 1;
...
void() spawn_supernova =
{
	...
	newmis.movetype = MOVETYPE_NOCLIP;
	newmis.avelocity = RandomVector('360.00000 360.00000 360.00000');
	newmis.velocity = ('0 0 1' * (self.lip / 3));
	newmis.lip = self.origin_z + self.lip;
...
void() supernova_init_think =
{
	if (time < self.splash_time)
	{
		if (self.lip == 0)
		{
			...
			//self.lip = (vlen(self.origin - trace_endpos) / 3.00000);
			if (trace_fraction < 1.00000)
				self.lip = (vlen(self.origin - trace_endpos) / 2.00000);
			else
				self.lip = 128;
		}
