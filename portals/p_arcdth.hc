...
void() arc_pulse_touch = {
	...
	thinktime self : random(0.02, 0.12);
...
void() arc_pulse_think = {
	...
	if (self.cnt > 0)
	{
		...
		if (r > 1.00000)
		{
			...
			//do_lightning ( self.owner, 1, STREAM_ATTACHED, 1.00000, self.origin, self.origin + randomv('-150 -150 -100', '150 150 100'), 3.00000);
		}
...
void() arc_transit = {
	...
	if (self.cnt > 40) {
		...
		if (self.classname == "top") {
			...
			particle2 ( (self.origin + randomv('-30 -30 -30', '30 30 30')), '-30.00000 -30.00000 50.00000', '30.00000 30.00000 100.00000', (144.00000 + random(15.00000)), 2, 80.00000);
			if (test == 0) {
				self.dmg = self.spelldamage*0.50000;
				if (self.enemy.status_effects & STATUS_WET)
					self.dmg *= 3.62500;
				
				do_lightning ( self, 3, STREAM_ATTACHED, 3.00000, self.origin, self.enemy.origin, (self.spelldamage*0.75000) + random(self.spelldamage*(-0.50000), self.dmg), TE_STREAM_LIGHTNING);
				arc_pulse(-800);
			}
